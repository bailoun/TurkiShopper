...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="Order" && $field=="Status")
	{
		if ($value == "Ordered") {  
$value =$value. ' <img width=50 height=50 border=0 src="ordered.png">';  
   $color="black";  
} else {  
   $value ='<b>'.$value. '</b> <img src="ordered.png">';  
   $color="red";  
}  
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="Shipment" && $field=="Shipment_Amount_\$")
	{
		return 0;
	}
				if($table=="Shipment" && $field=="Shipment_Amount_TL")
	{
		return 0;
	}
				if($table=="Sent" && $field=="Transaction_date")
	{
		return now();;
	}
	return "";
